...
- Name: e2e.spec.ts >> /extract renders entity spans for a known input
- Location: tests\e2e.spec.ts:14:5
...
Locator: locator('[data-testid="entity-span"]').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="entity-span"]').first()

```

```yaml
- main:
  - heading "Extract — Named Entity Recognition" [level=1]
  - textbox "Paste text to extract named entities from...": Akira Kurosawa directed Seven Samurai in 1954.
  - button "Extract"
